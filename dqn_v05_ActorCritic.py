...
class A2CAgent:
    def __init__(self, state_size, action_size, load_state):
        ...
        if self.load_model:
            ...
            self.actor.load_weights("dqn_actor.h5")
            self.critic.load_weights("dqn_critic.h5")
    ...
    def build_actor(self):
        ...
        actor = Sequential()
        actor.add(Dense(512, input_dim=self.state_size, activation='relu', kernel_initializer='he_uniform'))
        actor.add(Dense(512,  activation='relu', kernel_initializer='he_uniform'))
        actor.add(Dense(512,  activation='relu', kernel_initializer='he_uniform'))
    ...
    def build_critic(self):
        critic = Sequential()
        critic.add(Dense(512, input_dim=self.state_size, activation='relu', kernel_initializer='he_uniform'))
        critic.add(Dense(512, activation='relu', kernel_initializer='he_uniform'))
        critic.add(Dense(512, activation='relu', kernel_initializer='he_uniform'))
...
class Env:
    ...
    def reset(self):
        ...
        while source == destination or destination in self.graph.cs_info:
            destination = self.graph.destination_node_set[
                np.random.randint(0, len(self.graph.destination_node_set) - 1)]
        ...
        self.path_info = ta.get_feature_state(self.sim_time, self.pev, self.CS_list, self.graph, NCS)

        # print('\npev soc', self.pev.SOC)
        state = [self.pev.curr_location / self.graph.num_node, self.pev.destination / self.graph.num_node,
                 self.pev.curr_SOC, self.sim_time / 288]
    ...
    def step(self, action):
        ...
        if len(fpath) > 1:
            ...
            for i in range(len(fpath) - 1):
                ...
                tnode = fpath[i + 1]
                # self.pev.path.append(self.pev.curr_location)
                # print(i, self.pev.curr_location)
                chargingprice = cs.price[int(self.sim_time / 5)]
                distance_onetwo = self.graph.distance(fnode, tnode)
                velocity_onetwo = self.graph.velocity(fnode, tnode, int(self.sim_time / 5))
                ...
                self.sim_time, time = ta.update_ev(self.pev, self.graph, self.pev.curr_location, tnode, self.sim_time)
                ...
                if self.pev.curr_SOC <= 0.0:
                    ...
                    reward = reward - 1 * road_cost

                    if self.timeIDX != int(self.sim_time / 5):
                        # print(i, self.pev.curr_location, 'idx charged')
                        self.timeIDX = int(self.sim_time / 5)
                        self.path_info = ta.get_feature_state(self.sim_time, self.pev, self.CS_list, self.graph,
                                                                   NCS)
                        next_node = self.pev.curr_location
                        next_state = [self.pev.curr_location / self.graph.num_node,
            ...
            self.path_info = ta.get_feature_state(self.sim_time, self.pev, self.CS_list, self.graph, NCS)
            next_node = self.pev.curr_location
            next_state = [self.pev.curr_location / self.graph.num_node, self.pev.destination / self.graph.num_node,
                          self.pev.curr_SOC, self.sim_time / 288]
            for path in self.path_info:
                cs, req_soc, driving_cost, fpath, rpath, fp_dist, rp_dist, fp_dt, rp_dt, fp_w, r_w, wtime, ctime, cs_char_cost, home_char_cost = path
                next_state += [wtime, ctime, driving_cost / 10, cs_char_cost / 10, home_char_cost / 10]
...
if __name__ == "__main__":
    ...
    resultdir = '{0:02}-{1:02} {2:02}-{3:02} {4:02} AC {5} {6}'.format(now_start.month, now_start.day, now_start.hour, now_start.minute, now_start.second,TRAIN, EPISODES)
    basepath = os.getcwd()
    ...
    if TRAIN:
        agent = A2CAgent(state_size, action_size, False)
        for e in range(EPISODES):
            episcore = 0
            epistep = 0
            numsucc, numfail = 0 , 0
            env = Env(graph_train, state_size, action_size)
            print("\nEpi:", e,  'episcore:', episcore)
        ...
        agent.actor.save("{}/dqn_actor.h5".format(resultdir))
        agent.actor.save("dqn_actor.h5")

        agent.critic.save("{}/dqn_critic.h5".format(resultdir))
        agent.critic.save("dqn_critic.h5")
...
###############################  performance evaluation  #############
    ...
    testagent = A2CAgent(state_size, action_size, True)
