...
class CS:
    def __init__(self, node_id, long, lat):
        ...
        self.TOU_price = [0.1736, 0.1601, 0.1748, 0.174, 0.1724, 0.1735, 0.1601, 0.1736, 0.1321, 0.1618, 0.1616, 0.1650,
                          0.161, 0.1635, 0.1650, 0.1633, 0.1749, 0.1808, 0.1808, 0.1753, 0.1739, 0.1717, 0.1823, 0.1786,
                          0.1823, 0.1321, 0.1687]
        for i in range(288):
...
class EV:
    def __init__(self, id, t_start, soc, source):
        self.id = id
...
class Env:
    ...
    def reset(self):
        ...
        self.path_info = []
        self.pev = EV(e, t_start, soc, source)
        self.sim_time = self.pev.t_start
        self.path_info = ta.get_feature_state_refer(self.sim_time, self.pev, self.CS_list, self.graph, self.action_size)
        # self.path_info = ta.get_feature_state_optimal_refer(self.sim_time, self.pev, self.CS_list, self.graph, self.action_size)
    ...
    def step(self, action):
        ...
        if len(path)>1:
            ...
            self.pev.expense_time_part = (self.pev.totaldrivingtime + self.pev.cschargingwaitingtime) * UNITtimecost
            self.pev.expense_cost_part = self.pev.totaldrivingdistance * self.pev.ECRate * self.pev.cschargingprice + self.pev.cschargingcost
            self.pev.totalcost = self.pev.expense_time_part + self.pev.expense_cost_part

            # self.pev.totalcost = self.pev.drivingtime * UNITtimecost + self.pev.drivingdistance * self.pev.ECRate*cs.price[int(self.sim_time/5)] + self.pev.chargingcost + self.pev.waitingtime * UNITtimecost
            ...
            reward = -1 * (self.pev.cschargingcost + self.pev.cschargingwaitingtime * UNITtimecost)
            # print(done, reward)
...
def test_dqn(EV_list_DQN_REF, CS_list_DQN_REF, graph, env, agent ):
    ...
    for e, pev in enumerate(EV_list_DQN_REF):
        ...
        score = 0
        path=[]
        state, source = env.test_reset(pev, graph, CS_list_DQN_REF)
        print("\nEpi:", e, agent.epsilon)
        print(source,'->')
...
###############################  performance evaluation  #############
    ...
    for i in range(10):
        ...
        test_dqn(EV_list_DQN_REF, CS_list_DQN_REF, graph, env, testagent)
        ...
        EV_list_optimal_ref = ta.optimal_solution(EV_list_optimal_ref, CS_list_optimal_ref, graph)
        ...
        ta.sim_result_text_last(i, CS_list, graph, resultdir, EV_list_DQN_REF=EV_list_DQN_REF, EV_list_Astar_ref=EV_list_Astar_ref, EV_list_optimal_ref=EV_list_optimal_ref, EV_list_Astar_shortest=EV_list_Astar_shortest)

        ta.sim_result_general_presentation_last(i, graph, resultdir, npev, EV_list_DQN_REF=EV_list_DQN_REF, EV_list_Astar_ref=EV_list_Astar_ref, EV_list_optimal_ref=EV_list_optimal_ref)
