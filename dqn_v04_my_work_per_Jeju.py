...
class Env:
    ...
    def reset(self):
        ...
        self.path_info = ta.get_feature_state_jeju(self.sim_time, self.pev, self.CS_list, self.graph, NCS)
            ...
            for n in range(100):
                ...
                while not done:
                    ...
                    if done:
                        ...
                        # print('({},{},{})'.format(env.pev.csid, env.pev.req_SOC, step))
                        # print(env.pev.path)
                        # xx, yy = [], []
                        # nth = 0
                        # for nid in env.pev.path:
                        #     x, y = env.graph.nodes_xy(nid)
                        ...
                        #
                        # if env.pev.cs != None:
                        #     cs_x, cs_y = env.graph.nodes_xy(env.pev.cs.id)
                        #     plt.plot(cs_x, cs_y, 'D', label=' EVCS')
                        #
                        # s_x, s_y = env.graph.nodes_xy(env.pev.source)
                        # # ax.set_xlim(graph.minx - 1, graph.maxx + 1)
                        # # ax.set_ylim(graph.miny - 1, graph.maxy + 1)
                        # plt.plot(s_x, s_y, 'p', label='Source')
                        #
                        # d_x, d_y = env.graph.nodes_xy(env.pev.destination)
                        # # plt.xlim(env.graph.minx - 0.01, env.graph.maxx + 0.01)
                        # # plt.ylim(env.graph.miny - 0.01, env.graph.maxy + 0.01)
                        # plt.plot(d_x, d_y, 'p', label='Destination')
                        # plt.legend()
                        #
                        #
                        # plt.show()
